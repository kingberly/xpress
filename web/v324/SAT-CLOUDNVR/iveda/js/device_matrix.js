...
function UpdateBoxContentHeight() {
    var $matrix = $('#device_matrix');
    var $frames = $matrix.find('._device_single_frame');
    ...
    var $frame = $frames.first(),
        $box = $frames.find('._matrix_boxcontent'),

        framePad = $frame.innerHeight() - $frame.height();

    var maxHeight = Math.floor($matrix.height() / 2);
    ...
    $frames.height( Math.min(height, maxHeight) );
}
...
function RefreshDisplay( mode, matrix_display_mode, order_by, shared_mode )
{
	...
	// get data
	$.getJSON(
		CONFIG.ROOT_URL + "backstage_device.php",
		{
			command: "list",
			mode: mode,
			order_by: order_by,
...
function refreshSinglePlayer(id){
    ...
    window.parent.$('#muteBtn').addClass('disabled').removeAttr( 'onclick' );
...
function RefreshAfterDelete( data )
	{
		...
		$.getJSON(
			...
			function( data )
			{
				parent.window.location = CONFIG.OEM_URL + "index.php?thishref=true";
			}
		);
	}
...
function ReDisplay( matrix_display_mode )
{
	...
	for( var i=0; i<device_list.length; i++ )
	{
		...
		if (device.sequential == "null" || device.sequential == null) continue;
		if (p2p_data[device.id]!= undefined) continue;

		var favoritecon = "_matrixbox_favorite_off";
		var lockicon ="_matrixbox_lock_off";
		var idstring = device.id.toString();
		...
		var nailpos = -1;
		/*if (favorite_mode) {
			nailpage = device.myfavorite_nail_page;
			nailpos = device.myfavorite_nail_position;
		}*/
		...
		if (device.sequential == null) device.sequential = device.id;
		var sequential_id = device.sequential;
		...
		if( device.service_type == "P2P" || device.device_type == "p2p");
		else
			refreshbutton = "";

		if (device.service_type == "nvr") {
			websettingbutton = "";
			fullscreenbutton = "";
			mutesettingbutton = "";
			refreshbutton = "";
		...
		//if (favorite_mode) sequential_id = device.myfavorite_seq;
		html_to_insert += "<div class='"+ basicframe + " " + disabled + "' seq_id='" + sequential_id + "' id='"+i+"' nail_position='" + nailpos +"'>"+
                            ...
                            // Matrix title bar
                            "<div class=\"_matrix_title_bar clearfix\">" +
                                "<div class='_matrix_title' title='"+device.name+"'>" +
                                    "<span style='" + publicstyle + "' class='"+ singal_default +"' title='"+t_js.device_matrix_MSG['0003']+"'></span>" +
                                    ReduceStrLength(device.name, 15) +
                                "</div>"+
                                "<div class='_matrix_iconbox' ref_uid='" + device.uid + "' ref_id='"+ device.id +"' >" +
                                    // icons
                                    //"<span style='"+ fav_publicstyle +"' class='" + favoritebutton + " " + favoritecon + "' title='"+ t_js.device_matrix_MSG['0001'] + "'></span>" +
                                    ...
                                    //"<span style='" + publicstyle + "' class='"+ refreshbutton +"' title='"+t_js.device_matrix_MSG['0007']+"' onclick='refreshSinglePlayer("+device.id+")'></span>" +
                                    //"<span style='" + publicstyle + "' class='"+ closebutton +"' title='Remove'></span>" +
                                "</div>" +
                                "<div class='_matrix_time'></div>" +
                            "</div>" +
		...
		if(true)
		{
			if( device.service_type == "WEB" && device.device_type == "") //old style, deprecated
			{
				html_to_insert += "<div class='device_matrix_y'>" +
                                      "<div class='icon_nb' onClick='PopupDeviceViewWindow(device_list[" + i + "]);' style='z-index:100;cursor:pointer;margin-top:10px;margin-left:60px;'></div>" +
                                  "</div>" ;
			}
			else if( device.device_type == "p2p" && (device.service_type == "camera" || device.service_type == "nvr"))
			{
                // Assign p2p data {{{
				var GetArray = PrepareDeviceGetParameters(device);
				var URL = CONFIG.ROOT_URL + 'device.php?' + GetArray + "&ToTab=0";

				//Get Quality from cookie, and set url_path
				var quality = GetCookie("quality_grp_" + device.uid);
				if (quality == "") {
					if (device.ME_URL != null && device.ME_URL != "null")
						quality = 1;
					else
						quality = 2;
				}
				...
				var _url_path = device.url_path;
				var _port = device.port;
				if (device.stream_url_path) {
					_url_path = device.stream_url_path;
					_port = device.stream_port;
				}
				else {
					if (quality == 0 && (device.HI_URL != null && device.HI_URL != "null")) _url_path = device.HI_URL;
					if (quality == 1 && (device.ME_URL != null && device.ME_URL != "null")) _url_path = device.ME_URL;
					if (quality == 2 && (device.LO_URL != null && device.LO_URL != "null")) _url_path = device.LO_URL;
				}
                ...
                var device_p2p = {
                    ...
                    features: fillEmptyString(device.features),
                    default_id: fillEmptyString(device.default_id),
                    default_pw: fillEmptyString(device.default_pw),
                    model_id: device.model_id,
                    model: fillEmptyString(device.model),
				...
				if (device.dataplan != "D") {
                    $.extend(device_p2p, {
                        url_prefix: device.url_prefix,
                        url_path: _url_path,
                        local_uri: device.local_uri,
                        stream_local_uri: device.stream_local_uri
                    });
				}
				...
				if (device.dataplan != "D") {
                    $.extend(device_p2p, {
                        uri: "", //ip address
                        tcp_socket_port: -1,
                        req_cm_port: device.CM_PORT,
                        req_rtsp_port: _port,
                        req_web_url: device.WBSS_URL
                    });

					if ( device.purpose == "WBNV" && !device.req_web_port)
						device_p2p.req_web_port = device.port;
					else
						device_p2p.req_web_port = device.req_web_port;
				}

				for (var j=0; j<nvr_list.length; j++) {
					if (nvr_list[j].linked == device.uid) {
						if (nvr_list[j].purpose == "WBNV") {
							device_p2p.nvr_id = nvr_list[j].uid;
							device_p2p.req_nvr_port = nvr_list[j].port;
						}
						else if (nvr_list[j].purpose == "WBNA") {
							device_p2p.req_nvr_port2 = nvr_list[j].port;
						}
					}
				}

				if (isFullscreening && i==0){
					isFullscreening_id = device.id;
					device_p2p.fullscreen = true;
                } else {
					device_p2p.fullscreen = false;
                }
                // }}}

				html_to_insert +=  '<div id="p2p_' + device.id + '"></div>';
                p2p_data[device.id] = device_p2p;
			}
			else if ( device.service_type == "REMOTE" )
			{
				html_to_insert += "<div class='matrix_remote' onClick='RemoteClick(" + i + ");' ></div>" ;
			}
			else if ( device.device_type == "syncme" || device.service_type == "syncme" ) //device.device_type == "syncme" : 1.0 rule
			{
				html_to_insert += "<div class='device_matrix_y'>" +
                                      "<div class='icon_nb' onClick='openSyncme(device_list[" + i + "]);' style='z-index:100;cursor:pointer;margin-top:10px;margin-left:60px;'></div>" +
                                  "</div>" ;
			}
            else {
                html_to_insert +=  GetViewFullItem(device);
            }
		}

		html_to_insert +=   "</div>"+ // ._matrix_boxcontent
						  "</div>";
	}

	// apply the html
    var $matrix = $('#device_matrix').html( html_to_insert );

	//dummy frame
	PutInDummy();

    UpdateBoxContentHeight()
...
function openControlPanel(i){ //Switch to Full Screen
	window.open(CONFIG.ROOT_URL + "java_panel.php?port="+i,"Ratting","width=130,height=60,menubar=0,status=0,location=0,resizable=0,scrollbars=0,toolbar=0");
}
			...
			$('#device_matrix ._matrix_iconbox').each(function(){
				...
				if (i.toString() == ref_id){ //id match
					...
					if (p2p_data[i]["service_type"] != 'nvr') {
                        ...
                        window.parent.$('#muteBtn').removeClass('disabled').attr( 'onclick', 'mutePlayer(' + i + ')' );
					}
...
function openp2pDevice(i){
	window.open(CONFIG.ROOT_URL + "device_java.php?remote_id=" + i);
}
...
function PutInDummy(){
    var dummy = "<div class='_device_single_frame ui-state-disabled' style=''>" +
                    "<div class=\"_matrix_title_bar clearfix\">" +
                        "<div class='_matrix_title isdummy'>&nbsp;&nbsp;</div>" +
                        "<div class='_matrix_iconbox isdummy'>" +
                            "<span class='_matrixbox_favorite_dummy'></span>" +
                            "<span class='_matrixbox_lock_dummy'></span>"+
                        "</div>"+
                    "</div>" +
                    "<div class='_matrix_boxcontent isdummy'></div>" +
                "</div>";

	var s_count = $('div#device_matrix').find('._device_single_frame').length;//var s_count = $('div#device_matrix').find('._device_single_frame').size();//jinho
	//alert(s_count);
...
function QuerySignal(shared_mode){
	...
	$.getJSON(
	CONFIG.ROOT_URL + "backstage_signal.php",
	{
		command: command,
	},
	function( data )
...
function RefreshInfoSystem() {
	...
	$.getJSON(CONFIG.ROOT_URL + "backstage_info_system.php", function(data) {
...
/**
 * Close current window and re-create parent players.
 */
function CloseWindow() {
    window.top.location.href = window.top.location.href.replace(/\/iveda\/.*/, "");
}
