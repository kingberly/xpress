 ...
 * reference from device_matrix.js 
 * /var/www/SAT-CLOUDNVR/js  
 *Writer: JinHo, Chang  
*****************/ 
/* jshint -W069, -W041 */
			...
			$('#device_matrix ._matrix_iconbox').each(function(){
				...
				if (i.toString() == ref_id){ //id match
					...
					if (p2p_data[i].owner_id == user_id) {
						web_button.removeClass('_matrixbox_web_setting_off').addClass('_matrixbox_web_setting').attr('onClick','oForm.elements["camip"].value=openWeb("'+uri+'",' + port + ');');
					}
					if (p2p_data[i]["service_type"] != 'nvr') { //jinho disable
						...
						fulls_button.removeClass('_matrixbox_fullscreen_off').addClass('_matrixbox_fullscreen_on').attr('onClick','openVLCPage("' + p2p_data[i]["uid"] + '");');//jinho add vlc player in camera_setting.php
						//fulls_button.removeClass('_matrixbox_fullscreen_on').addClass('_matrixbox_fullscreen_off').removeAttr( 'onClick' );//remove button
						var fulls_button = $(this).find('.muteButton').eq(0);
						//fulls_button.addClass('_matrixbox_mute_setting_on_silent').attr('onClick','mutePlayer(' + i + ');');
						fulls_button.removeClass('_matrixbox_mute_setting_on_silent').removeClass('_matrixbox_mute_setting_on_sound').addClass('_matrixbox_mute_setting_off').attr('title',t_js.device_matrix_MSG['0006']).removeAttr( 'onClick' );
					}
					...
					$(applet_nvr).removeClass("applet_nvr_off").addClass("applet_nvr_on").attr('onClick','oForm.elements["camip"].value=openWeb("'+uri+'",' + port + ');');
					return false;
				}
			});
		}
...
function openWeb(uri,i){
	...
	alert("Camera Tunnel IP>> "+uri + ":" + i); //jinho replace and return
	return uri + ":" + i;
}
...
function PutInDummy(){
	...
	var s_count = $('div#device_matrix').find('._device_single_frame').size();
	//alert(s_count);
