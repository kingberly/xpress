...
$_GET['oem_id']="T04"; //for taipei project
...
//for taipei project site account login only
    ...
    //    if ( (preg_match("/^[0-9A-Z]{7-15}$/",$_REQUEST['user_name'])) //Dec-2
    // and ($_REQUEST['user_pwd'] == USER_PWD  )   )
    if ( (preg_match("/^[0-9A-Z]{8,15}$/",$_REQUEST['user_name'])) //Jan-11
       and ($_REQUEST['user_pwd'] == USER_PWD  )   )
    { //correct site format
    }else if ( (preg_match("/^[0-9-]{9,15}$/",$_REQUEST['user_name'])) //Jan-11
       and ($_REQUEST['user_pwd'] == USER_PWD  )   )
