...
function PrepareDeviceModifyParameterArray()
{
	...
	// check name //jinho fix QXP-277, /["'\[+&#]/g
	if( parameter_array.name.trim() == '' || parameter_array.name.search(/["'\[+&#]/g) != -1 )
	{
	...
	// check mac_addr
	if( ((parameter_array.mac_addr.trim() == "") || verifyMAC(parameter_array.mac_addr) == false) )
	{
		parameter_array.mac_addr = '';
		
		// check identifier
		if( parameter_array.identifier == '' )
		{
			Alert( t_js.device_common_MSG['0003'] );
			return null;
		}
	}
	else parameter_array.identifier = '';
	
	// check port
	if( verifyPort(parameter_array.port) == false )
	{
		Alert( t_js.device_common_MSG['0005'] );
		return null;
	}
...
{	
	...
	$('span#owner_id-span').attr('value',device_entry.owner_id);
