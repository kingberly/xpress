...
<!--remark--meta http-equiv="x-ua-compatible" content="ie=8" / -->
...
<link href='css/css_family=Source+Sans+Pro.css' rel='stylesheet' type='text/css'>
<link href="css/bootstrap.min.css" rel="stylesheet">
<link href="css/stick-footer.css" rel="stylesheet">
<link rel="stylesheet" href="css/normalize.css">
<link rel="stylesheet" href="css/main.css">
<script src="js/vendor/modernizr-2.6.2.min.js"></script>
<!--Added New CSS jj end-->
<!-- .3597 new added -->   
...
<script type="text/javascript">
    ...
    //	var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);	   
		...
		{
			// show welcome msg
      // remark and Added New CSS jj
			//$('#welcome_msg_prefix').html(t_js.index_MSG.Welcome);
			//$('#header_my_account').html(user_name);
			//$('#header_logout').html(t_js.index_MSG.Logout);
			var welcome_msg = "<?php echo _("Welcome"); ?> <span id='login_id' >" + user_name + "</span>! " +
				"<?php printf(_("This is your %s times login."), number_format($_SESSION["user_login_count"])); ?>";
			$("a#menu_loginout-a, a#footer_loginout").html("<?php echo _("Logout"); ?>");
			$("a#menu_loginout-a, a#footer_loginout").attr("href", "logout.php");
      // remark and Added New CSS jj end
      ...
      // remark and Added New CSS jj
			...
			//$("#logoinfo").hide();
			$("#welcome_msg-h1").html("");
			$("a#menu_loginout-a, a#footer_loginout").html("<?php echo _("Login"); ?>");
			$("a#menu_loginout-a, a#footer_loginout").attr("href", "login.php");
			// remark and Added New CSS jj end
...
<body>
  ...
  <?php if (isset($_SESSION['user_name'])): ?>
	<nav>
		<ul class="list-unstyled main-menu">
			...
			<?php 
				...
				elseif (isset($_SESSION['user_name']))
					$name = $_SESSION['user_name'];
        echo "<span class='icon glyphicon glyphicon-user'></span> ". _("Welcome").", <span id='login_id'>$name</span>! "
					/*. sprintf(_("This is your %s times login."), number_format($_SESSION["user_login_count"]))*/;
			?>
			</span>
			</div></li>
			<li class="<?php echo $oem_style_list['quick_start_off']; ?>"><a href="" id="footer_quick_start" onClick="<?php echo $oem_style_list['quick_start_onclick']; ?>" class="state-active open_page "><?php echo _("Quick Start"); ?></a></li>
				<?php if (isset($_SESSION['user_id']) && $_SESSION['user_id'] !="") { echo '<li><a href="" id="footer_help" onclick="window.open(\'p2p_diagnosis.php\');" class="state-active open_page">' . _("Help") . '<span class="icon glyphicon glyphicon-question-sign"></span></a></li>';} ?>
				<?php if (isset($oem_style_list['extra_top_links']) && $oem_style_list['extra_top_links'] != "") {echo $oem_style_list['extra_top_links'];}?>
			<li id="menu_device"><a href="device_matrix.php?mode=<?php echo $mode;?>" onClick="toActive(this.id);" class="navi_link open_page bottom_menu state-active open_page"><?php echo _("Live View"); ?><span class="icon glyphicon glyphicon-facetime-video"></span></a></li>
                        <li id="menu_setting"><a class="navi_link open_page bottom_menu" href="device_list.php?mode=<?php echo $mode;?>" onClick="toActive(this.id);"><?php echo _("Setting"); ?><span class="icon glyphicon glyphicon-wrench"></span></a></li>
                        <li id="menu_service" class="navi_link open_page bottom_menu <?php echo $oem_style_list['menu_service_off']; ?>" href="service_matrix.php" onClick="toActive(this.id);">Other Service</li>
					<?php
						if (isset($_SESSION["user_group_id"]) && $_SESSION["user_group_id"]==0){
							echo '<li id="menu_analysis" style="display:none;" class="navi_link open_page bottom_menu" onClick="toActive(this.id);" href="user_list.php?mode=' . $mode . '">Analysis</li>';
							echo '<li id="menu_user_list" style="display:none;" class="navi_link open_page bottom_menu" onClick="toActive(this.id);" href="user_list.php?mode=' . $mode . '">Admin</li>';
						}
					?>
			<li id="menu_share" ><a class="navi_link open_page bottom_menu" href="shared_matrix.php?mode=<?php echo $mode;?>" onClick="toActive(this.id);"><?php echo _("Share");?><span class="icon glyphicon glyphicon-share"></span></a></li>
			<li id='menu_support'><a class="navi_link open_page bottom_menu" href="Support.php?mode=<?php echo $mode;?>" onClick="toActive(this.id);"><?php echo _("Support");?><span class="icon glyphicon glyphicon-info-sign"></span></a></li>
			<li><a href="my_account.php" id="footer_my_account" class="state-active open_page"><?php echo _("Account"); ?><span class="icon glyphicon glyphicon-cog"></span></a></li>
			<li><a href="login.php" id="footer_loginout" class="state-active open_page"><?php echo _("Login"); ?><span class="icon glyphicon glyphicon-log-out"></span></a></li>
	...
	<div class="navbar navbar-inverse navbar-fixed-top">      
		...
		<?php if (isset($_SESSION['user_name'])): ?><div></div><?php else: ?>
		<a class="navbar-brand" href="#"><div id="<?php echo $oem_style_list['logo_id']; ?>" class="<?php echo $oem_style_list['logo_class']; ?>"></div></a>
		<?php endif; ?>
		<?php if (isset($_SESSION['user_name'])): ?>
		<div class="navbar-header pull-right">
		  <a id="nav-expander" class="nav-expander fixed">
		   &#9776; &nbsp; <div id="<?php echo $oem_style_list['logo_id']; ?>" class="<?php echo $oem_style_list['logo_class']; ?>"></div><span class="icon-bar"></span>
		<span class="icon-bar"></span>
		<span class="icon-bar"></span><i class="fa fa-bars fa-lg white"></i>
		  </a>
		</div>
		<?php endif; ?>
		...
		<div id='language'>
			...
			<label>
				<select id="user_language-select" class="form-control">
				  <option value="cs_CZ">Čeština</option>
				  <option value="de_DE">Deutsch</option>
				  <option selected="selected" value="en_US">English</option>
				  <option value="fr_FR">Français</option>
				  <option value="es_ES">Español</option>
				  <option value="es_US">Español (Mexico)</option>
			...
			<div class="videoframe">				
					...
					<div class="video_border">
						...
						<div id="v_relay" class="v_relay_alert_layer ">
							...
							<div class="btn_2_layer">
								<div class="btn_sharef" id="btn_relay_reconnect" onclick="";>Reconnect</div>
							</div>
						</div>
				...
				<div class="buttonbar">
					...
					<div class="mode_block">
						...
						<span id="mode_1" class="mode_button" onclick="change_video_mode(1);"><?php echo _("Play back");?></span>
    	...
    	<div class="bannerMain">
		  ...
		  <label>
            <select id="user_language-select">
              ...
              <option value="fr_FR">Français</option>
              <option value="es_ES">Español</option>
              php //<option value="es_US">Español (Estados Unidos)</option>
              <option value="it_IT">Italiano</option>
              <option value="ja_JP">日本語</option>
              <option value="ko_KR">한국어</option>
              <option value="nl_NL">Nederlands</option>
	...
	<div id="xmlresult" style="display:none;"></div>
		...
		<!-- /container -->
		 <div class="footer">
			<div class="container">
			  <div id="footer_logo" style="<?php echo $oem_style_list['footer_logo_style']; ?>" class="<?php echo $oem_style_list['footer_logo_class']; ?>"><?php echo $oem_style_list['footer_logo_html']; ?></div>
			<div id="copyright"><?php echo "Version " . SVW_VERSION . SVN_REVISION . "<br />&copy;" . $oem_style_list['copyright_html']; ?></div>
