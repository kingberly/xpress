<?php
/****************
 *Validated on Oct-7,2015
 * added auto update online list for 3 minutes 
 *Writer: JinHo, Chang   
*****************/
//ini_set('memory_limit', '64M'); //qet data from query_info
include("../../header.php");
include("../../menu.php");
...
//jinho updated for new account_list
$sql="select * from isat.query_info where mac_addr in ('{$tmp}')";
...
for($j=0;$j<$num_mac_list;$j++)
{
        ...
        */
        ...
        if ($db_mac_list["is_signal_online"]=='false')//new added
          $check_list[$db_mac_list["mac_addr"]][status]=0;
        else $check_list[$db_mac_list["mac_addr"]][status]=1;
...
///
        ...
        //for new account_list
        if(!in_array($value_cam[device_uid],$uid_list)) // check register or not
						echo "<td><font color=#000000>Unregistered</font></td>\n";
        else if ($check_list[substr($value_cam[device_uid],-12)][status]==0)
          echo "<td><font color=#AAAA33> ( Off-line )</font></td>\n";
        /*elseif($value_cam[dataplan]=="LV" or $value_cam[dataplan]=="D" or $value_cam[dataplan]=="EV" ) //no dataplan in the isat.query_info
          echo "<td ><font color=#33AAAA>Package Not Recording</font> </td>\n";
        */
        else  echo "<td ><font color=#0033FF><B>Online</B></font></td>\n";
