...
#
        $sql="select user_name,mac_addr from isat.query_info where mac_addr='{$_REQUEST["mac"]}' limit 0,1";
        sql($sql,$result_tmp,$num_tmp,0);
        fetch($db_tmp,$result_tmp,0,0);
       $sql="select * from qlync.application_info where Role_type='01' and Owner='{$db_tmp["user_name"]}' and Time_start > '".(date("Y")-1911)."-".date("m")."-".date("d")."' order by Time_start asc limit 0,2";
        sql($sql,$result_list,$num_list,0);
if(is_file($home_path."/html/scid/{$_REQUEST["scid"]}/01/{$db_tmp["user_name"]}/cover.jpg"))
{
	//echo "<img src=\"/html/scid/{$_REQUEST["scid"]}/01/{$db_tmp["user_name"]}/cover.jpg\" width=120 style=\"float: left; margin: 15px\">\n";
    echo "<img src=\"/html/scid/{$_REQUEST["scid"]}/01/{$db_tmp["user_name"]}/cover.jpg\" width=200 >\n";
}
