<?php
/****************
 *Validated on Nov-27,2017,
 * log tunnel connection number in /tmp/tunnel_connection_xx.log
 * use setOnly to reduce database query
 * fix monthly log switch
 * wget -q -O - "http://chimeiManageUser:cHsCtmeQYNFJwsCTffux@127.0.0.1/manage/manage_tunnelserver.php?command=get_channels&tunnelserver=829cdda3ebaa4cb7a75aa949d4836ef1"     
 *Writer: JinHo, Chang
*****************/
ini_set('memory_limit', '128M'); //T04 20170928 fail
...
if (isset($tunnelArray) and $flag_writelog){
  foreach($tunnelArray as $tuid)
  {
      ...
      }else die("getTunnelArray:".$cn_camera['error_msg']);
  }
}
fclose($fplog);
curl_close($ch);
