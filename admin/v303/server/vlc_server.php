<?PHP
        if($_REQUEST["step"]=="aa")
        {
                header("Refresh: 10;"); //jinho extend
        }
        else
        {
                header("Refresh: 30;"); //jinho extend
...
//                      $rec[status][$value_rec[uid]]=$value_rec[status];
		...
		echo "<td nowrap>Total : ".sizeof($cn_camera[$key][cameras])."\n";
			echo "<BR>\n";
			echo "( <font color=#0000FF>Streaming : {$stream_count}</font> )\n";
		echo "</td>\n";
    }else{//jinho
    ...
    echo "<font color=red>CPU : NA<br>\nMem : NA</font><br>\n"; 
		...
		echo "<td>\n";
			if($_REQUEST["step"] <> "show_camera" or $_REQUEST["server_id"]<> $value[id])
			{
				if($value["license_id"] <> "")
				{
					//echo "<a href=vlc_server.php?server_id={$value[id]}&step=show_camera target=_blank>Show Camera</a>\n";
...
$sql="select * from isat.query_info where mac_addr in ('{$tmp}') group by mac_addr";
...
//        $check_list[$db_mac_list["Mac"]][ip]=$db_mac_list["Ip"];
	$uid_list[]=$db_mac_list["uid"];
        $check_list[$db_mac_list["mac_addr"]][fw]=$db_mac_list["model"]." ({$db_mac_list["device_models_version"]})";
        $check_list[$db_mac_list["mac_addr"]][ip]=$db_mac_list["ip_addr"];
	if($db_mac_list["is_signal_online"] == 'true')
	{
		$c_list[]=$db_mac_list["uid"];
	}
...
{
         echo "function show() {\n";
                echo "var option = document.getElementById(\"category\").value;";
                echo "hideall();";
 foreach($cn_camera[$key][cameras] as $key_cam=>$value_cam)

        {
                ...
                {
                        echo "if(option == \"Unregistered\")";
                        echo "{";
                                echo "var o{$value_cam[id]}=document.getElementById(\"{$value_cam[id]}\");\n";
                                echo "o{$value_cam[id]}.style.display=(o{$value_cam[id]}.style.display=='none') ? '' : 'none';\n";
                        echo "}";
                }
                elseif(!in_array($value_cam[device_uid],$c_list))
                {
                        echo "if(option == \"Offline\")";
                        echo "{";
                                echo "var o{$value_cam[id]}=document.getElementById(\"{$value_cam[id]}\");\n";
                                echo "o{$value_cam[id]}.style.display=(o{$value_cam[id]}.style.display=='none') ? '' : 'none';\n";
                        echo "}";
        ...
        foreach($cn_camera[$key][cameras] as $key_cam=>$value_cam)
{
        {
                echo "var o{$value_cam[id]}=document.getElementById(\"{$value_cam[id]}\");\n";
                echo "o{$value_cam[id]}.style.display='none';\n";
...
echo "</script>\n";
			...
			echo "<tr class=topic_main>\n";
				...
				echo "<td> Recycle Days </td>\n";
                                echo "<td colspan=4>\n";
                                        echo "<select id=\"category\" onchange=\"show()\">\n";
                                                echo "<option value=''>Catagory</option>\n";
                                                echo "<option value=Streaming>Streaming</option>\n";
                                                echo "<option value=Offline>Offline</option>\n";
                                                echo "<option value=Unregistered>Unregistered</option>\n";
                                                echo "<option value=NStreaming>Not Streaming</option>\n";
                                                echo "<option value=Not>No Service</option>\n";
			...
			echo "</tr>\n";
				...
				$row=0;
				foreach($cn_camera[$key][cameras] as $key_cam =>$value_cam){
	                        echo "<tr class=tr_".($row++%2+1)." id='{$value_cam[id]}'>\n";
					...
					elseif($value_cam[dataplan]=="LV" or $value_cam[dataplan]=="D" or $value_cam[dataplan]=="EV" )
					{
                                                echo "<td colspan=4><font color=#33AAAA>Recording Package Not Service</font> </td>\n";
					}
					elseif($rec[$value_cam[device_uid]][status]=="Streaming")
					{
