...
function bindCamera ($mac,$ac,$user) //can return true/false
{
    global $oem, $api_id, $api_pwd, $api_path;
...
{
  if ( isset($_REQUEST["mac"]) and isset($_REQUEST["ac"]) and isset($_REQUEST["user"]) )
  {
          if (bindCamera($_REQUEST["mac"],$_REQUEST["ac"],$_REQUEST["user"]) ){
              InsertLog($_REQUEST["mac"]."->".$_REQUEST["user"],"SUCCESS");
              echo 'Success';
          }else{
            InsertLog($_REQUEST["mac"]."->".$_REQUEST["user"],"FAIL");
            echo 'Fail';
          }
