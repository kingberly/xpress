...
if ($_SESSION['Contact']!=""){
	$myID=getEndUserID($_SESSION['Contact']);
	$where = " and visitor_id={$myID} ";
}else {
	echo "Contact Info Not Set!!!";
	exit();
}
if($_REQUEST["step"]=="search")
{
	if($_REQUEST["account"] <> "")
	{
		...
		$where.=" and query_share.user_name like '%".mysql_real_escape_string($_REQUEST["account"])."%' or query_share.reg_email like '%".mysql_real_escape_string($_REQUEST["account"])."%' ";
	}
	if($_REQUEST["uid"] <> "")
	{
	...
	if($_REQUEST["uid"] <> "" and $_REQUEST["account"] <> "")
	{
		...
		$where.=" and ( query_share.user_name like '%".mysql_real_escape_string($_REQUEST["account"])."%' or query_share.reg_email like '%".mysql_real_escape_string($_REQUEST["account"])."%')  and mac_addr like '%".mysql_real_escape_string($_REQUEST["uid"])."%' ";
	}
}
...
//        else
        {
                $where.=" and  left(group_id,".(strlen((int)$_SESSION["SCID"])).")='".(int)($_SESSION["SCID"].$_SESSION["AID"])."' and length(group_id) >1";
        }
}
...
$sql="select count(distinct(query_share.reg_email)) as Ec,user.group_id from isat.query_share,isat.user where 1=1 and user.id=query_share.visitor_id {$where} ";
...
$sql = "select count(distinct(uid)) as total  from isat.query_share";
...
$sql="select distinct(query_share.user_name),user.group_id from isat.user,isat.query_share where 1=1 and query_share.user_name=user.name {$where} group by query_share.user_name order by query_share.user_name asc limit {$start},{$sql_stop}";
...
$sql="select query_share.user_name,query_share.reg_email,mac_addr,query_share.name,uid,ip_addr,model,device_models_version from isat.query_share,isat.user where query_share.user_name in ('{$t2}')  {$where} group by query_share.mac_addr,uid order by query_share.user_name asc ";
if (isset($_REQUEST['debugadmin'])) echo "\n(array:)".$sql;
...
echo "<table  class=table_main>";
	echo "<tr class=tr_2>";
		...
		//jinho add title
		if ($_SESSION['Contact']!=""){
			echo "<h3>{$_SESSION['Contact']}被分享攝影機"._("Total")." "._("Account").": ".$total_dev;
...
$row=1;
	...
	foreach($account as $key=>$value)
	{
        ...
        if ($_SESSION["ID_admin_qlync"]!='1')
            ...
            echo "<a href='#' onclick=\"window.open('/plugin/debug/playback_list.php?user={$key}&mac=".$value[mac][$li]."','',config='height=650,width=600')\">".$value[mac][$li]."</a>";
