...
//      $where =" where Email like '%".mysql_real_escape_string($_REQUEST["account"])."%' or Account like '%".mysql_real_escape_string($_REQUEST["account"])."%' ";
	if($_REQUEST["account"]<> "")
		$where.=" and user_name like '%".mysql_real_escape_string($_REQUEST["account"])."%' ";
	...
	if($_REQUEST["uid"] <> "")//jinho fix uid ambigious issue query_info.
	        ...
	        $where .=" and query_share.uid like '%".mysql_real_escape_string($_REQUEST["uid"])."%' ";
}
...
$sql="select query_share.id,user.group_id from isat.query_share,isat.user where is_signal_online ='true' and visitor_id=user.id  {$where}  group by mac_addr";
...
$sql="select * from isat.query_share,isat.user,isat.signal_server_online_client_list  where is_signal_online='true' and user.id=query_share.visitor_id and query_share.uid=signal_server_online_client_list.uid {$where} group by mac_addr order by user_name asc limit {$start},{$sql_stop}";
//echo $sql;
...
echo $gid[substr($user_gid[$i],3,1)][(int)substr($user_gid[$i],4,2)][0];
        ...
        if ($_SESSION["ID_admin_qlync"]!='1')
          ...
          echo "<a href='#' onclick=\"window.open('/plugin/debug/online_player.php?user={$account[$i]}&uid=".substr($uid[$i],0,18)."','',config='height=450,width=500')\">".substr($uid[$i],0,18)."</a>";
